...
def load_data():
    ...
    for _, user in df.iterrows():
        try:
            projects_val = user["projects"]
            if isinstance(projects_val, str):
                projs = json.loads(projects_val)
            else:
                continue
        except Exception:
            continue
...
def load_model():
    if os.path.exists("model.pkl"):
        try:
            ...
            print(f"Warning: Failed to load model.pkl ({e}). Training a new model on the fly...")
            try:
                ...
                print(f"Error: Failed to train model on the fly: {train_err}")
                return None
    return None
...
def build_context_summary() -> str:
    ...
    top_users = pd.Series(proj_df.groupby("username")["project_id"]
...
class QueryEngine:
    ...
    def execute(self, query: str) -> str:
        ...
        if re.search(r"highest|most projects|maximum projects|top user", q):
            top = pd.Series(self.proj.groupby("username")["project_id"]
                            .count()).sort_values(ascending=False).head(5)
